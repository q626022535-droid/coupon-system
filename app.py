...
def get_db():
    ...
    if _client is None:
        if not MONGO_URI:
            raise ValueError("请设置 MONGO_URI 环境变量")
        # 添加连接选项，延迟连接避免启动时 DNS 问题
        _client = MongoClient(
            MONGO_URI,
            connect=False,  # 延迟连接
...
def generate_session_token(user_id):
    ...
    get_db().users.update_one({"_id": ObjectId(user_id)}, {"$set": {"session_token": token}})
    return token

def clear_session_token(user_id):
    get_db().users.update_one({"_id": ObjectId(user_id)}, {"$set": {"session_token": None}})
